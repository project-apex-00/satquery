...
function updateModeView(mode) {
  ...
  if (mode === "single") {
    ...
    modeIndicatorBadge.className = "badge badge-success";
    singleUploadContainer.classList.add("hidden");
    dualUploadContainer.classList.remove("hidden");
...
function renderAnalysisResults(data) {
  ...
  if (data.task_type === "bi_temporal_change") {
    ...
    totalChangeVal.textContent = `${spec.total_change_percentage || 0}%`;
    vegLossVal.textContent = `-${spec.vegetation_loss_percentage || 0}%`;
    builtupGainVal.textContent = `+${spec.built_up_gain_percentage || 0}%`;
  } else if (data.task_type === "cross_modal_sar_optical") {
    ...
    fusionBuiltupVal.textContent = `${spec.built_up_coverage_percentage || 0}%`;
    fusionWaterVal.textContent = `${spec.water_coverage_percentage || 0}%`;
    fusionCloudVal.textContent = `${spec.optical_cloud_coverage_percentage || 0}%`;
    fusionPenetratedVal.textContent = `${spec.radar_cloud_penetration_percentage || 0}%`;
  } else {
